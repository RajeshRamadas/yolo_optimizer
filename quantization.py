...
def run_qat(model, data_path, epochs=5, img_size=640):
    ...
    try:
        ...
        logging.info(f"Quantization-aware training completed, mAP50: {results.metrics.get('mAP50(B)', 0.0):.4f}")
...
def quantize_model_fp16(model_path, dataset_path, quantized_model_dir):
    ...
    try:
        ...
        model.export(format="onnx", imgsz=640, dynamic=True, simplify=True, device=device, half=half)
